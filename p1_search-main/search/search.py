...
def depthFirstSearch(problem):
    ...
    while not stack.isEmpty():
        ...
        if problem.isGoalState(current_state):
            return actions
        if current_state not in l_visited:
            l_visited.append(current_state)
            successors = problem.getSuccessors(current_state)
            for state, action, cost in successors:
                if state not in l_visited:
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not pri_queue.isEmpty():
        ...
        if current_state not in l_visited:
            ...
            for state, action, cost in successors:
                if state not in l_visited:
                    pri_queue.push((state, actions + [action]), problem.getCostOfActions(actions + [action])+ heuristic(state, problem))
    return []
